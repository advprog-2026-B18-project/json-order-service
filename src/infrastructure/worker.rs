...
async fn try_consume(
    ...
    info!("✅ [worker] siap consume dari '{QUEUE_NAME}'");
...
async fn handle_delivery(
    ...
    match result {
        ...
        Err(e) => {
            ...
            if is_permanent {
                warn!("⚠️ [worker] permanent failure, ack untuk discard: {e}");
            ...
            if retry_count >= MAX_RETRY {
                warn!("⚠️ [worker] max retry tercapai, kirim ke dead letter");
                let _ = delivery
...
async fn process_message(
    ...
    {
        warn!(
            "⚠️ [worker] duplicate message order_id={}, skip",
            request.order_id
        );
        return Ok(());
    }
    ...
    if order.status != OrderStatus::Reserving {
        warn!(
            "⚠️ [worker] order_id={} sudah bukan Reserving (status={}), skip",
