...
pub async fn submit_rating(
    ...
    info!(
        "⭐ [submit_rating_product] order_id={} titipers_id={}",
    ...
    let order = order_repo
        ...
        .ok_or_else(|| {
            warn!(
                "⚠️ [submit_rating_product] order tidak ditemukan: {}",
                order_id
            );
    ...
    if order.status != OrderStatus::Completed {
        warn!(
            "⚠️ [submit_rating_product] order belum COMPLETED, status={:?}",
            order.status
        );
    ...
    {
        warn!(
            "⚠️ [submit_rating_product] rating product sudah ada untuk order_id={}",
            order_id
        );
    ...
    info!(
        "✅ [submit_rating_product] rating saved rating_id={}",
    ...
    debug!(
        "📦 [submit_rating_product] notifying Inventory module product_id={}",
        product_id
    );
    ...
    {
        error!(
            "⚠️ [submit_rating_product] send_product_rating gagal (non-fatal): {:?}",
            e
        );
    } else {
...
pub async fn get_rating(
    ...
    rating_product_repo
        ...
        .ok_or_else(|| {
            warn!(
                "⚠️ [get_rating] rating belum ada untuk order_id={}",
                order_id
            );
