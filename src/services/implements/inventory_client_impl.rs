...
async fn manage_stock(
    ...
    let (status, _) =
        internal_post(&url, json!({ "order_id": order_id, "quantity": quantity })).await?;
...
pub(crate) async fn confirm_order_received(
    product_id: Uuid,
    order_id: Uuid,
) -> Result<(), AppError> {
    debug!(
        "✅ [inventory] confirm_order_received product_id={} order_id={}",
        product_id, order_id
    );
    ...
    match status {
        200 => {
            debug!("✅ [inventory] confirm berhasil product_id={}", product_id);
            Ok(())
        }
        404 => {
            warn!(
        ...
        409 => {
            debug!(
                "ℹ️ [inventory] sudah di-confirm sebelumnya order_id={} (idempotent)",
                order_id
            );
            Ok(())
        }
