...
pub(crate) async fn send_jastiper_rating(
    ...
    match status {
        200 => {
            debug!("✅ [user] rating jastiper berhasil dikirim jastiper_id={}", jastiper_id);
            Ok(())
        }
        404 => {
            debug!("⚠️ [user] jastiper tidak ditemukan jastiper_id={} (non-fatal)", jastiper_id);
            Ok(())
        }
        409 => {
            debug!("ℹ️ [user] rating sudah dikirim sebelumnya order_id={} (idempotent)", order_id);
            Ok(())
        }
        code => {
            error!("❌ [user] send_jastiper_rating unexpected status={} jastiper_id={}", code, jastiper_id);
            Ok(())
        }
    }
}