...
pub async fn submit_rating_jastiper(
    ...
    info!(
        "⭐ [submit_rating_jastiper] order_id={} titipers_id={}",
        order_id, titipers_id
    );
    ...
    let order = order_repo
        ...
        .ok_or_else(|| {
            warn!(
                "⚠️ [submit_rating_jastiper] order tidak ditemukan: {}",
                order_id
            );
    ...
    if order.status != OrderStatus::Completed {
        warn!(
            "⚠️ [submit_rating_jastiper] order belum COMPLETED, status={:?}",
            order.status
        );
    ...
    {
        warn!(
            "⚠️ [submit_rating_jastiper] rating jastiper sudah ada untuk order_id={}",
            order_id
        );
    ...
    info!(
        "✅ [submit_rating_jastiper] rating jastiper saved rating_id={}",
        rating.rating_jastiper_id
    );

    debug!(
        "👤 [submit_rating_jastiper] notifying Profile module jastiper_id={}",
        order.jastiper_id
    );
    ...
    {
        error!(
            "⚠️ [submit_rating_jastiper] send_jastiper_rating gagal (non-fatal): {:?}",
            e
        );
    } else {
...
pub async fn get_rating(
    ...
    debug!(
        "🔍 [get_rating] order_id={} requester_id={}",
    ...
    if order.titipers_id != requester_id && order.jastiper_id != requester_id {
        warn!(
            "⚠️ [get_rating] forbidden: requester_id={} bukan titipers/jastiper",
            requester_id
        );
        return Err(AppError::Forbidden(
            "Anda tidak memiliki akses ke pesanan ini".to_string(),
    ...
    rating_jastiper_repo
        ...
        .ok_or_else(|| {
            warn!(
                "⚠️ [get_rating] rating jastiper belum ada untuk order_id={}",
                order_id
            );
